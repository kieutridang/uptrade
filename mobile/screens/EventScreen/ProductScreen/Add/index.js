...
import { MULTI_FILE_UPLOAD_MUTATION, MUTATION_CREATE_PRODUCT } from '../../../ProductScreen/product.typedef'
...
import { costList, descriptionList, logisticList } from '../../../ProductScreen/Add/fieldList'
...
class AddProductScreen extends React.Component {
  ...
  render () {
    const { activeSections, isSupplierOpen, openSupplierModal,
      setFieldValue, values, navigation, userInfo } = this.props
    let fieldSetting = null
    if (userInfo && userInfo.mobileProductSetting && userInfo.mobileProductSetting.availableField) {
      fieldSetting = userInfo.mobileProductSetting.availableField
    }
...
function productForm (values, eventId, imageUrls, supplier) {
  return {
    essentials: {
...
async function createProductOnline (props, values) {
  try {
    ...
    const eventName = navigation.getParam('eventName', null)
    ...
    var product = productForm(values, eventId, uploadedImageUrls, newSupplier)
    ...
    if (response.data && response.data.createProduct) {
      ...
      await store.push(`products_${userInfo._id}_${eventId}`, createdProduct)
...
async function createProductOffline (props, values) {
  try {
    ...
    const eventId = navigation.getParam('eventId', null)
    const eventName = navigation.getParam('eventName', null)
    var product = productForm(values, eventId, values.imageUrl, values.supplier)
    product = {
      ...product,
      _id,
      isNew: true,
    ...
    await store.push(`products_${userInfo._id}_${eventId}`, product)
    // push id product to event
    const eventsOffline = await store.get(`events_${userInfo._id}`)
    const eventPosition = eventsOffline.findIndex(event => event._id === eventId)
    if (eventPosition > -1) {
      ...
      eventsOffline[eventPosition].updatedAt = new Date()
    }
    await store.delete(`events_${userInfo._id}`).then(() => {
      store.save(`events_${userInfo._id}`, eventsOffline)
    })
    ...
    console.warn('EventScreen/ProductScreen/Add/index.js', 'createProductOffline', error)
  }
}
...
function setSuppliersOffline (props) {
  try {
    ...
    console.warn('EventScreen/ProductScreen/Add/index.js', 'setSuppliersOffline', error)
  }
}
...
function checkContainField (fieldSetting, list) {
  try {
    ...
    console.warn('EventScreen/ProductScreen/Add/index.js', 'checkContainField', error)
  }
}
...
function filterSection (fieldSetting) {
  try {
    ...
    console.warn('EventScreen/ProductScreen/Add/index.js', 'filterSection', error)
  }
}
