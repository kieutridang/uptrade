...
class ConfirmResetPasswordScreen extends React.Component {
      ...
      <KeyboardAvoidingView style={{ flex: 1, position: 'relative' }} behavior='padding' enabled>
        <ScrollView keyboardShouldPersistTaps='always' style={{ flex: 1, position: 'relative', backgroundColor: 'white' }}>
          ...
          >
            ...
            <Text style={{ marginTop: 70, marginLeft: 30, fontSize: 32, color: 'white', fontWeight: 'bold' }}>Reset Password</Text>
          </View>
...
const enhance = compose(
  ...
  withFormik({
    ...
    handleSubmit: async (values, { setSubmitting, setErrors, props }) => {
      const { resetPasswordUser, navigation } = props
      ...
      if (values.password !== values.passwordConfirm) {
        ...
        try {
          ...
          if (response && response.data && response.data.confirmPasswordReset) {
            // const { user, token } = response.data.setupPasswordUser
            DropdownHolder.alert('success', 'Reset password successfully', 'Welcome to Uptrade, please sign in to proceed')
...
export default hoistStatics(enhance)(ConfirmResetPasswordScreen)
