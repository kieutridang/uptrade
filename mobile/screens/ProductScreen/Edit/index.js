...
import { MULTI_FILE_UPLOAD_MUTATION, QUERY_PRODUCT_DETAIL, MUTATION_EDIT_PRODUCT } from '../product.typedef'
...
const enhance = compose(
  ...
  graphql(MUTATION_EDIT_PRODUCT, {
    name: 'editProduct',
    options: {
      refetchQueries: ['ProductList', 'GetProductDetail'],
  ...
  withFormik({
    ...
    mapPropsToValues: (props) => {
      const { isConnected, productsOffline, navigation, queryProductDetail, eventProductsOffline } = props
      const productDetail = queryProductDetail.product || {}

      const productsInStore = mergeProductsAllTypes(eventProductsOffline, productsOffline)
      ...
      if ((currentProductInStore && currentProductInStore.isNew) || !isConnected) {
        return {
          category: _.get(currentProductInStore, 'essentials.category', 'Agriculture') || 'Agriculture',
          subCategory: _.get(currentProductInStore, 'essentials.subCategory', 'Agricultural Growing Media') || 'Agricultural Growing Media',
          brand: _.get(currentProductInStore, 'essentials.brand', 'Samsung') || 'Samsung',
          itemStatus: _.get(currentProductInStore, 'essentials.itemStatus', 'SOURCING') || 'SOURCING',
          ...
          testCertificate: _.get(currentProductInStore, 'essentials.testCertificate', 'No') || 'No',
          ...
          // event
          eventName: _.get(currentProductInStore, 'eventName', null)
        }
      }
      ...
      if (isConnected) {
        return {
          ...
          // event
          eventName: _.get(productDetail, 'eventName', null)
        }
      }
    ...
    handleSubmit: async (values, { setSubmitting, setErrors, props }) => {
      const { productsOffline, navigation, isConnected, eventProductsOffline } = props
      ...
      let product = {
        ...
        supplier: values.supplier.map(item => {
          return {
            name: item.name,
            _id: item._id,
...
async function editProductOnline (props, product, values, setErrors, currentProductInStore) {
  ...
  try {
    ...
    if (response.data && response.data.updateProduct) {
      if (response.data.updateProduct && response.data.updateProduct.eventId) {
        let productsOffline = await store.get(`products_${userInfo._id}_${product.eventId}`) || []
        let products = consolidateDataOnlineAndOffline([response.data.updateProduct], productsOffline)
        await store.delete(`products_${userInfo._id}_${product.eventId}`)
        await store.save(`products_${userInfo._id}_${product.eventId}`, products)
      } else {
        let productsOffline = await store.get(`products_${userInfo._id}`)
        let products = consolidateDataOnlineAndOffline([response.data.updateProduct], productsOffline)
        await store.delete(`products_${userInfo._id}`)
        await store.save(`products_${userInfo._id}`, products)
      }
...
async function editEventProductOffline (props, product, values, id, currentProductInStore) {
  const { userInfo, isConnected } = props
  try {
    let eventProductsOffline = []
    let eventProducts = await store.get(`products_${userInfo._id}_${product.eventId}`)
    eventProductsOffline = (eventProducts || []).map((element) => {
      if (id === element._id) {
        return {
          ...
          ...product,
          status: 'EVENTS',
          eventName: currentProductInStore.eventName,
    ...
    return store.delete(`products_${userInfo._id}_${product.eventId}`).then(() => {
      return store.save(`products_${userInfo._id}_${product.eventId}`, eventProductsOffline)
    }).then(() => {
      if (!isConnected && currentProductInStore && !currentProductInStore.isNew) {
    ...
    console.warn('ProductSceen/Edit/index.js', 'editEventProductOffline', error)
  }
}
...
async function editProductOffline (props, product, values, id, currentProductInStore) {
  ...
  try {
    ...
    console.warn('ProductSceen/Edit/index.js', 'editProductOffline', error)
  }
}
...
function setSuppliersOffline (props) {
  try {
    ...
    console.warn('ProductSceen/Edit/index.js', 'setSuppliersOffline', error)
  }
}
